...
| `render.py` | EDL → mp4, local ffmpeg (enforces RULES §A) | `edl.json` + cached clips → `ad.mp4` | ⬜ Phase 1 |
| `selfcheck.py` | boundary frames/waveforms + flags | `ad.mp4` + `edl.json` → `selfcheck/` packet | ⬜ Phase 1 |
| `timeline_view.py` | filmstrip + waveform PNG at a point | clip + range → PNG | ⬜ Phase 1 (port from video-use) |
